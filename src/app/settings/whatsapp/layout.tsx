...
/**
 * Access gate for /settings/whatsapp.
 *
...
export default async function GatedLayout({ children }: { children: React.ReactNode }) {
  const { allowed } = await checkRouteAccess("/settings/whatsapp");
  if (!allowed) {
    return <AccessDenied what="Integrations" permissionLabel="Manage connections & keys" />;
  }
  return <>{children}</>;
}
